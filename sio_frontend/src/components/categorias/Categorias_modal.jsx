//sio_frontend/src/components/categorias/Categorias_modal.jsx
import React, { useEffect, useState } from 'react';
import { X, Save, Trash2, RotateCcw } from 'lucide-react';
import toast from 'react-hot-toast';
import { crearCategoria, actualizarCategoria, eliminarCategoria } from '../../apis/categorias/categoriasApi';

export default function Categorias_modal({ abierto, onClose, itemSelect, onGuardado }) {
  const [nomCat, setNomCat] = useState('');
  const [guardando, setGuardando] = useState(false);
  const esActualizar = Boolean(itemSelect);

  useEffect(() => {
    setNomCat(itemSelect ? itemSelect.nom_cat : '');
  }, [itemSelect, abierto]); 
  ...
  const manejarGuardar = async (e) => {
    ...
    try {
      if (esActualizar) {
        await actualizarCategoria(itemSelect.id_cat, { nom_cat: nomCat });
        toast.success("Actualizado");
      } else {
        await crearCategoria({ nom_cat: nomCat });
  ...
  const manejarEliminar = async () => {
    ...
    setGuardando(true);
    try {
      await eliminarCategoria(itemSelect.id_cat);
      ...
      <aside className={`fixed top-0 right-0 h-full w-full sm:w-[420px] bg-base-100 border-l border-white/15 shadow-xl z-50  transition-transform duration-300 ${abierto ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
        <form onSubmit={manejarGuardar} className="flex flex-col h-full">
          <div className="flex items-center justify-between px-5 py-4 border-b border-base-300">
            <h2 className="font-display font-bold">{esActualizar ? 'Actualizar' : 'Nueva Categoría'}</h2>
            <button type="button" onClick={onClose} className="btn btn-ghost btn-sm btn-circle"><X size={18} /></button>
          </div>
          <div className="flex-1 p-5">
            <label className="form-control w-full">
              <div className="label"><span className="label-text">Nombre de Categoría *</span></div>
              <input type="text" className="input input-bordered w-full uppercase" required value={nomCat} onChange={(e) => setNomCat(e.target.value.toUpperCase())} />
            </label>
          </div>
