//sio_frontend/src/components/tabla_bitacora/tipo_solicitudes_modal.jsx
import React, { useEffect, useState } from 'react';
import { X, Save, Trash2, RotateCcw } from 'lucide-react';
import toast from 'react-hot-toast';
import { crearTipoSolicitud, actualizarTipoSolicitud, eliminarTipoSolicitud } from '../../apis/tabla_bitacora/tabla_bitacoraApi';

export default function TipoSolicitudes_modal({ abierto, onClose, itemSelect, onGuardado }) {
  const [nomSolicitud, setNomSolicitud] = useState('');
  const [guardando, setGuardando] = useState(false);
  const esActualizar = Boolean(itemSelect);

  useEffect(() => {
    setNomSolicitud(itemSelect ? itemSelect.nom_solicitud : '');
  }, [itemSelect, abierto]); 
  ...
  const manejarGuardar = async (e) => {
    ...
    try {
      if (esActualizar) {
        await actualizarTipoSolicitud(itemSelect.id_solicitud, { nom_solicitud: nomSolicitud });
        toast.success("Actualizado");
      } else {
        await crearTipoSolicitud({ nom_solicitud: nomSolicitud });
  ...
  const manejarEliminar = async () => {
    ...
    setGuardando(true);
    try {
      await eliminarTipoSolicitud(itemSelect.id_solicitud);
      ...
      <aside className={`fixed top-0 right-0 h-full w-full sm:w-[420px] bg-base-100 border-l border-white/15 shadow-xl z-50  transition-transform duration-300 ${abierto ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
        <form onSubmit={manejarGuardar} className="flex flex-col h-full">
          <div className="flex items-center justify-between px-5 py-4 border-b border-base-300">
            <h2 className="font-display font-bold">{esActualizar ? 'Actualizar' : 'Nueva Solicitud'}</h2>
            <button type="button" onClick={onClose} className="btn btn-ghost btn-sm btn-circle"><X size={18} /></button>
          </div>
          <div className="flex-1 p-5">
            <label className="form-control w-full">
              <div className="label"><span className="label-text">Nombre de Solicitud *</span></div>
              <input type="text" className="input input-bordered w-full uppercase" required value={nomSolicitud} onChange={(e) => setNomSolicitud(e.target.value.toUpperCase())} />
            </label>
          </div>
