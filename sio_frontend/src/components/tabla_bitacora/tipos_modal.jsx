//sio_frontend/src/components/tabla_bitacora/tipos_modal.jsx
import React, { useEffect, useState } from 'react';
import { X, Save, Trash2, RotateCcw } from 'lucide-react';
import toast from 'react-hot-toast';
import { crearTipo, actualizarTipo, eliminarTipo } from '../../apis/tabla_bitacora/tabla_bitacoraApi';

export default function Tipos_modal({ abierto, onClose, itemSelect, onGuardado }) {
  ...
  useEffect(() => {
    setNomTipo(itemSelect ? itemSelect.nom_tipo : '');
    setDescTipo(itemSelect?.desc_tipo ? itemSelect.desc_tipo : '');
  }, [itemSelect, abierto]);
  ...
  const manejarGuardar = async (e) => {
    ...
    setGuardando(true);
    try {
      const payload = { nom_tipo: nomTipo, desc_tipo: descTipo };
      if (esActualizar) {
        await actualizarTipo(itemSelect.id_tipo, payload);
  ...
  const manejarEliminar = async () => {
    ...
    setGuardando(true);
    try {
      await eliminarTipo(itemSelect.id_tipo);
      ...
      <aside className={`fixed top-0 right-0 h-full w-full sm:w-[420px] bg-base-100 border-l border-white/15 shadow-xl z-50 transition-transform duration-300 ${abierto ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
        <form onSubmit={manejarGuardar} className="flex flex-col h-full">
          <div className="flex items-center justify-between px-5 py-4 border-b border-base-300">
            <h2 className="font-display font-bold">{esActualizar ? 'Actualizar' : 'Nuevo Tipo'}</h2>
            <button type="button" onClick={onClose} className="btn btn-ghost btn-sm btn-circle"><X size={18} /></button>
          </div>
          <div className="flex-1 p-5 space-y-4">
            <label className="form-control w-full">
              <div className="label"><span className="label-text">Nombre del Tipo *</span></div>
              <input type="text" className="input input-bordered w-full uppercase" required value={nomTipo} onChange={(e) => setNomTipo(e.target.value.toUpperCase())} />
            </label>
            <label className="form-control w-full">
              <div className="label"><span className="label-text">Descripción</span></div>
              <input type="text" className="input input-bordered w-full uppercase" value={descTipo} onChange={(e) => setDescTipo(e.target.value.toUpperCase())} />
            </label>
          </div>
